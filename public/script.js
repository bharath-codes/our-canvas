window.addEventListener('load', () => {
    ...
    function handleUndo() { if (historyIndex > 0) { historyIndex--; redrawCanvas(historyIndex); updateButtonStates(); } }
    function handleRedo() { if (historyIndex < history.length - 1) { historyIndex++; redrawCanvas(historyIndex); updateButtonStates(); } }
    function handleUndoClick() { handleUndo(); socket.emit('undo'); }
    function handleRedoClick() { handleRedo(); socket.emit('redo'); }
    ...
    function stopDrawing() { if (!isDrawing) return; isDrawing = false; ctx.beginPath(); saveState(); }
    function clearLocalCanvas() { ctx.clearRect(0, 0, canvas.width, canvas.height); }
    function handleClearClick() { clearLocalCanvas(); saveState(); socket.emit('clear'); }
    socket.on('clear', () => { clearLocalCanvas(); saveState(); });
